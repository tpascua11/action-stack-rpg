// ============================================================
//  EnemyZone — Top section showing enemy card with HP bar
// ============================================================

export default function EnemyZone({ enemy, isShaking }) {
  const hpPct = Math.max(0, (enemy.health / enemy.max_health) * 100);
  const isDead = enemy.health <= 0;

  return (
    <div className="h-[22%] flex items-center justify-center border-b border-red-900/30"
      style={{ background: 'radial-gradient(circle at center, #2a1520 0%, #0f0f1a 100%)' }}>

      <div className={`flex flex-col items-center gap-2 transition-opacity duration-500 ${isDead ? 'opacity-30' : 'opacity-100'}`}>
